...
export default function Login() {
    ...
    const onSubmitSocialLogin = async (data) => {
        const res = await getDaynamicPostData('socialLogin', { email: data.email, name: data.name ,socialLoginId:data.userID})
        setLoginRes(res)
        setOpen(true);
        if (res?.status) {
            const userData = res?.user_data;
                ...
                <div className="form-field-form">
                    <form onSubmit={handleSubmit(onSubmit)}>
                        ...
                        <div >
                            ...
                            /> */}
                            <FacebookLogin
    appId="884136509011330"
    //autoLoad={true}
    fields="name,email,picture"
    onClick={(e) => { console.log('onclick',{e})}}
    callback={(e) => { onSubmitSocialLogin(e)}} />
                        </div>
